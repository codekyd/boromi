...
export const createNewLoanRequest = ( loanRequest) => async dispatch => {
    dispatch({type: CREATE_LOAN_REQ_REQUEST})
    try {
        const res = await axios.post('/api/loanRequests',loanRequest)
        dispatch({
...
// Dispatches an action to  get all the loan requests of a specific User
export const getLoanRequestsByUserID = ( userID ) => async dispatch => {
    dispatch({ type: GET_LOAN_REQ_OF_USER_BY_ID_REQUEST });
    try {
        const res = await axios.get(`/api/users/${userID}/loanRequests`)
        dispatch({
            type: GET_LOAN_REQ_OF_USER_BY_ID_SUCCESS,
            payload: res.data.loanRequests
        })
    } catch (err) {
        dispatch({
            types: GET_LOAN_REQ_OF_USER_BY_ID_FAILURE,
            payload: err.response.data
        })
    }
}
